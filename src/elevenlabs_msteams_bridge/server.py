...
from .elevenlabs import ElConnector
...
class BridgeServer:
    ...
    def __init__(self, cfg: BridgeConfig, connect_el: ElConnector | None, vision: VisionDescriber | None | str) -> None:
        self.cfg = cfg
        self._connect_el = connect_el
        self._vision = vision
    ...
    async def _handle_upgrade(self, request: web.Request) -> web.StreamResponse:
        ...
        # Reject rather than spin up a second billed ElevenLabs conversation.
    ...
    async def start(self) -> None:
        ...
        await site.start()
        log.info(
            f"elevenlabs-msteams-bridge listening on {self.cfg.host}:{self.cfg.port} "
            f"(agent {self.cfg.elevenlabs_agent_id}, host {self.cfg.el_host})"
        )
...
async def start_server(
    ...
    """Start the bridge and return its handle. `vision` defaults to the
    env-configured describer; pass an async callable to supply your own model,
    or None to disable vision path 2."""
    server = BridgeServer(cfg, connect_el, vision)
    await server.start()
    return server
